...
proc handleRequest(req: Request) {.async, gcsafe.} =
  ...
  try:
    ...
    # Get all tasks
      ...
      let tasks = getAllTasksOrdered(db) # Paged logic inside getAllTasksOrdered removed/needs adjustment?
 # Wait, I changed getAllTasksOrdered to return all. Paging needs manual slice.
 # Or I can use queryRowsTask(db, "LIMIT ... OFFSET ...")
 # But helper logic is cleaner.
 # Slicing in memory for now (simpler refactor).
...
proc mainAsync(configFilename: string) {.async.} =
  ...
  monitor.startMonitoring(cfg.smtp)
