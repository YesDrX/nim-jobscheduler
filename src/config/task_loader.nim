import streams, os, strutils, options, logging
import std/md5
...
proc calculateConfigHash(taskConfig: TaskConfigYAML): string =
  ## Calculate MD5 hash of task configuration for change detection
  return $toMD5($taskConfig)
  ...
  for path in walkDirRec(tasksDir):
    if path.endsWith(".yaml") and fileExists(path):
      ...
      try:
        ...
        error "Error loading task file: ", path
      ...
      if taskYaml.task.scheduleType.isSome:
        try:
          scheduleTypeEnum = parseEnum[ScheduleType](taskYaml.task.scheduleType.get())
        except:
          info "Invalid schedule type in ", path, ": ", taskYaml.task.scheduleType.get()
          continue

      # Calculate config hash for change detection
      let configHash = calculateConfigHash(taskYaml.task)
      
      # Resolve paths
      let resolvedCalendarPath = resolveCalendarPath(taskYaml.task.calendarPath.get(""), path)
      ...
      if existingTasks.len > 0:
        ...
        if tData.configHash == configHash and tData.calendarPath == resolvedCalendarPath:
          ...
          info "Updated task '", t.name, "' with calendarPath: '", t.calendarPath, "'"
