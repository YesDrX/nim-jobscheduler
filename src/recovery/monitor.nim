...
proc checkJob(monitor: ProcessMonitor, execId: int,
    execution: Execution, smtpConfig: SmtpConfig) {.async.} =
  try:
...
proc checkRunningJobs(monitor: ProcessMonitor,
  ...
  try:
    ...
    for row in runningExecutions:
      futures.add(monitor.checkJob(row.dbId, row.data, smtpConfig))
...
proc monitorLoop(monitor: ProcessMonitor, smtpConfig: SmtpConfig) {.async.} =
  monitor.running = true
  while monitor.running:
    await monitor.checkRunningJobs(smtpConfig)
    await sleepAsync(monitor.checkIntervalSeconds * 1000)

proc startMonitoring*(monitor: ProcessMonitor, smtpConfig: SmtpConfig) =
  if not monitor.running:
    asyncCheck monitor.monitorLoop(smtpConfig)
