...
proc getRecentExecutionsPaged*(db: DbConn, limit: int, offset: int): seq[
    ExecutionSummary] =
  # Join query needed. Manual SQL because macro doesn't do joins.
  # Use LEFT JOIN to include executions even if job/task was deleted
  var query = """
    SELECT e._dbID, e.pid, e.startTime, e.endTime, e.status, COALESCE(j.name, 'Unknown'), COALESCE(t.name, 'Unknown'), COALESCE(t._dbID, 0)
    FROM ExecutionTable e 
    LEFT JOIN JobTable j ON e.jobId=j._dbID 
    LEFT JOIN TaskTable t ON j.taskId=t._dbID
...
proc getExecutionsCount*(db: DbConn): int =
  # Use LEFT JOIN to count all executions including orphaned ones
...
proc updateExecutionStatus*(db: DbConn, id: int, status: ExecutionStatus,
    errorMsg: string = "") =
  ...
  if errorMsg == "":
    ...
    db.exec(sql"UPDATE ExecutionTable SET status = ?, errorMessage = ? WHERE _dbID = ?",
        $status, errorMsg, id)
...
proc getExecutionsRunningOrScheduled*(db: DbConn): seq[tuple[dbId: int,
    data: Execution]] =
  return queryRowsExecution(db, "status IN ('" & $esRunning & "', '" &
      $esScheduled & "')")

proc getExecutionById*(db: DbConn, id: int): Option[tuple[dbId: int,
    data: Execution]] =
...
proc getRunningExecutionsSummary*(db: DbConn): seq[ExecutionSummary] =
  ## Get running executions with job/task info for dashboard
  let query = """
    SELECT e._dbID, e.pid, e.startTime, e.endTime, e.status, COALESCE(j.name, 'Unknown'), COALESCE(t.name, 'Unknown'), COALESCE(t._dbID, 0)
    FROM ExecutionTable e 
    LEFT JOIN JobTable j ON e.jobId=j._dbID 
    ...
    WHERE e.status = ?
    ORDER BY e.startTime DESC"""
  let rows = db.getAllRows(sql(query), $esRunning)
  var res: seq[ExecutionSummary] = @[]
  for row in rows:
    var s = ExecutionSummary()
    s.id = parseInt(row[0])
